<script>
	...
	const calculate = () => {
		...
		const arrowBottomTransform = ``;
		const arrowTopTransform = ``;
		const arrowLeftTransform = ``;
		const arrowRightTransform = ``;
...
<div
	...
	class="content"
	style="padding: {padding}; z-index: {zIndex + 10}; {positionStyle}"
>
	{#if closeButton}
		<button class="button-close" on:click={setOpen}>
			<svg
				width="18"
				height="17"
				viewBox="0 0 18 17"
			...
			>
				<path d="M17 16L1 1" stroke="#5B626A" stroke-width="1.75" />
				<path d="M1 16L17 1" stroke="#5B626A" stroke-width="1.75" />
			</svg>
		</button>
	{/if}
	<slot />
	{#if arrow}
		<div
			bind:this={arrowRef}
			class="arrow"
			style="position: absolute; color: {arrowColor}; {arrowStyleProps};"
		>
			<!-- ◥ -->
		</div>
	{/if}
