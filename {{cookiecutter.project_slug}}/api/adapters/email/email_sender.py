...
class EmailSender:
    ...
    @classmethod
    def get_unsub_url(cls, email):
        """
        Get an unsubscribe token for the user to remove them from the mailing list
        :param email:
        :return:
        """
        # Encode the token to unsubscribe the user
        s = URLSafeSerializer(global_config.SECRET_KEY, salt=global_config.UNSUB_SALT)
        return f'{global_config.SERVER_URL}/unsubscribe/{s.dumps(email)}'

    @staticmethod
    def render_newsletter_email(subject, main_header, main_subheading, main_img_url, btn_url='', btn_txt=''):
        """
        Queue an email using the newsletter format with an optional CTA button
        :param subject:
        :param main_header:
        :param main_subheading:
        :param main_img_url:
        :param btn_url:
        :param btn_txt:
        :return:
        """
        config_dict = dict(main_header=main_header,
                           main_subheading=main_subheading,
                           main_img_url=main_img_url,
                           btn_url=btn_url,
                           btn_txt=btn_txt,
                           subject=subject)
        template = EmailTemplatesDAO.get('newsletter', email_params=config_dict)
        return template

    def send_newsletter_email(self, to_list, subject, main_header,
                              main_subheading, main_img_url, btn_url='', btn_txt=''):
        """
        Queue an email using the newsletter format with an optional CTA button
        :param to_list:
        :param subject:
        ...
        :return:
        """
        template = self.render_newsletter_email(subject=subject, main_header=main_header,
                                                main_subheading=main_subheading, main_img_url=main_img_url,
                                                btn_url=btn_url, btn_txt=btn_txt)
        self.send_email_template(email_template=template, to_list=to_list)

    def send_order_confirmation(self, target_email):
        """
        Queue an email explaining that the order has been submitted to and accepted by the printer
        :param target_email:
        :return:
        """
        main_subheading = "Your order has been submitted and accepted by our printer. " \
                          "We'll get started on it as soon as possible!"
        self.send_newsletter_email(to_list=[target_email],
                                   subject='Your Luminary Handbook order confirmation',
                                   main_header='Your order has been submitted',
                                   main_subheading=main_subheading,
                                   main_img_url='https://blog.luminaryhandbook.com/allbooks-stacked-new-1000.jpg')

    def send_user_confirmation_email(self, target_user: User):
        """
        Send an email with a link the user can click that confirms that they have access to that account.
        :param target_user: User with the email that is being confirmed
    ...
    def send_password_reset_email(self, target_user: User):
        """
        Send a password reset email to a targeted user.
        This includes a reset_url link that directs back to an authenticated page where a new password can be set.
        :param target_user:
        :type target_user:
        :return:
        :rtype:
        """
        email_params = self.get_params(target_user=target_user)
        template = EmailTemplatesDAO.get('password_reset', email_params=email_params)
        self.send_email_template(email_template=template, to_list=[target_user.email])
